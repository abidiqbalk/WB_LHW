...
class SupportMeeting < PhoneEntry

	has_one :detail, :class_name => "SupportMeetingDetail"	# so all phone-entries have a common interface

	def self.import_data
		puts "Importing support group meeting"
		ft = GData::Client::FusionTables.new 
		ft.clientlogin(Yetting.fusion_account,Yetting.fusion_password)		
		support_meeting_google_table = ft.show_tables[5]
		last_record = self.order("meta_submission_date").last
		
		if last_record.nil?
			puts "nil record case got run"
			new_records = support_meeting_google_table.select "*", "ORDER BY '*meta-submission-date*' ASC"
		else
			last_record = self.order("meta_submission_date").last
			...
			new_records = support_meeting_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end
		fields = support_meeting_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					...
					new_support_meeting.build_detail(
						:lhw_code => record[fields[11][:name].downcase.to_sym],
						:count => record[fields[12][:name].downcase.to_sym],
						:members_group_a => record[fields[13][:name].downcase.to_sym],
						:members_group_b => record[fields[14][:name].downcase.to_sym],
						:members_group_c => record[fields[15][:name].downcase.to_sym],
						:members_group_d => record[fields[16][:name].downcase.to_sym],
					)

					new_support_meeting.save!
					unless new_support_meeting.photo_url.nil?
						new_support_meeting.update_attribute(:photo,open(new_support_meeting.photo_url))
					end
		...
		puts "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " support_meeting records."
	end
end
