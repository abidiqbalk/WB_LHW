...
=begin
Wraps data-collection functionality for special_tasks.
...
=end
class SpecialTask < PhoneEntry

	has_one :detail, :class_name => "SpecialTaskDetail"	# so all phone-entries have a common interface
...
=end
	def self.import_data
		...
		special_task_google_table = ft.show_tables[8]

		last_record = self.order("meta_submission_date").last
		
		if last_record.nil?
			puts  "nil record case got run"
			new_records = special_task_google_table.select "*", "ORDER BY '*meta-submission-date*' ASC"
		else
			...
			new_records = special_task_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end

		fields = special_task_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					...
					new_special_task.build_detail(
						:dpiu_visited=> record[fields[11][:name].downcase.to_sym],
						:lhs_visited=> record[fields[12][:name].downcase.to_sym],
						:lhw_visited=> record[fields[13][:name].downcase.to_sym],
						:task_assigned=> record[fields[14][:name].downcase.to_sym],
						:report=> record[fields[15][:name].downcase.to_sym]
						
					)

					new_special_task.save!
		...
		puts  "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " special_task records."
	end
...
@param [Array of Objects holding the statistics] special_task_records these objects contain the necessary statistics that will be attached. 
@return [Array of Objects implementing Reportable Module] the collection object with attached statistics 
=end
	def self.build_statistics(special_task_records,collection)
		for unit in special_task_records
