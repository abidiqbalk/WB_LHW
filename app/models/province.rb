...
class Province < ActiveRecord::Base
	...
	def indicator_statistics(end_time)
		...
		self.statistics["Assessment"] = AssessmentDetail.find_by_sql("SELECT start_time as 'date', AVG(assessment_details.students_grade3) as 'students_grade3_average', AVG(assessment_details.students_grade4) as 'students_grade4_average',AVG(assessment_details.students_grade5) as 'students_grade5_average', AVG(assessment_details.teachers_present) as 'teachers_present_average', AVG(assessment_details.tasks_identified) as 'tasks_identified_average' FROM `assessment_details` INNER JOIN `phone_entries` ON `phone_entries`.`id` = `assessment_details`.`assessment_id` AND `phone_entries`.`type` IN ('Assessment') WHERE (`phone_entries`.`start_time` >= '2012-01-01 00:00:00') Group BY MONTH(start_time)")
		self.statistics["Mentoring"] = 	MentoringDetail.find_by_sql("SELECT start_time as 'date',AVG(mentoring_details.score_indicator1) as 'score_indicator1_average', AVG(mentoring_details.score_indicator2) as 'score_indicator2_average', AVG(mentoring_details.score_indicator3) as 'score_indicator3_average', AVG(mentoring_details.score_indicator4) as 'score_indicator4_average', AVG(mentoring_details.students_present) as 'students_present_average', AVG(mentoring_details.report_cards_issued) as 'report_cards_issued_average', AVG(mentoring_details.tasks_completed) as 'tasks_completed_average', AVG(mentoring_details.teachers_present) as 'teachers_present_m_average' FROM `mentoring_details` INNER JOIN `phone_entries` ON `phone_entries`.`id` = `mentoring_details`.`mentoring_id` AND `phone_entries`.`type` IN ('Mentoring') WHERE  (`phone_entries`.`start_time` >= '2012-01-01 00:00:00') Group BY MONTH(start_time)")
	end
...
=end	
	...
	def districts_with_compliance_statistics(start_time,end_time,number_of_months,districts) #returns stats on individual districts of a province
		...
		assign_compliance_statistics(districts,activities_conducted,adjusted_activities_conducted,schools_assigned,number_of_months)
