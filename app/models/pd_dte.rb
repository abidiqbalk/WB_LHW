class PdDte < PhoneEntry

	has_one :detail, :class_name => "PdDteDetail"	# so all phone-entries have a common interface

	def self.import_data
		puts "Importing pd_dte"
		ft = GData::Client::FusionTables.new 
		ft.clientlogin(Yetting.fusion_account,Yetting.fusion_password)		
		pd_dte_google_table = ft.show_tables[1]
		
		last_record = self.order("meta_submission_date").last
		
		if last_record.nil?
			puts "nil record case got run"
			new_records = pd_dte_google_table.select "*", "ORDER BY '*meta-submission-date*' ASC"
		else
			...
			new_records = pd_dte_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end
		fields = pd_dte_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					...
					new_pd_dte.build_detail(
						:conducted_at => record[fields[11][:name].downcase.to_sym],
						:education_officer => record[fields[12][:name].downcase.to_sym],
						:dtsc_head_te => record[fields[13][:name].downcase.to_sym],
						:present_dtes => record[fields[14][:name].downcase.to_sym],
						:present_ltes => record[fields[15][:name].downcase.to_sym],
						:activity_monitored_by => record[fields[16][:name].downcase.to_sym],
					)

					new_pd_dte.save!
					unless new_pd_dte.photo_url.nil?
						new_pd_dte.update_attribute(:photo,open(new_pd_dte.photo_url))
					end
		...
		puts "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " pd_dte records."		
	end
