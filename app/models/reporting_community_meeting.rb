...
=begin
Wraps data-collection functionality for reporting_community_meetings.
...
=end
class ReportingCommunityMeeting < PhoneEntry

	has_one :detail, :class_name => "ReportingCommunityMeetingDetail"	# so all phone-entries have a common interface
...
=end
	def self.import_data
		...
		reporting_community_meeting_google_table = ft.show_tables[ft.show_tables.index{|x|x.name=="Reporting - Community Meeting"}]
		...
		if last_record.nil?
			...
			new_records = reporting_community_meeting_google_table.select "*", "ORDER BY '*meta-submission-date*' ASC"
		else
			...
			new_records = reporting_community_meeting_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end

		fields = reporting_community_meeting_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					new_reporting_community_meeting = self.new(
					...
					new_reporting_community_meeting.save!
					unless new_reporting_community_meeting.photo_url.nil?
						new_reporting_community_meeting.update_attribute(:photo,open(new_reporting_community_meeting.photo_url))
					end
		...
		puts  "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " reporting_community_meeting records."
	end
...
@param [Array of Objects holding the statistics] reporting_community_meeting_records these objects contain the necessary statistics that will be attached. 
@return [Array of Objects implementing Reportable Module] the collection object with attached statistics 
=end
	def self.build_statistics(reporting_community_meeting_records,collection)
		for unit in reporting_community_meeting_records
			instance = collection.find { |instance| instance.name == unit.name }
			#attr_accessor_with_default is deprecated :S
			instance.reporting_community_meeting_count_total = unit.reporting_community_meeting_count_total_c.to_i
