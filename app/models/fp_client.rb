...
=begin
Wraps data-collection functionality for fp_clients.
...
=end
class FpClient < PhoneEntry

	has_one :detail, :class_name => "FpClientDetail"	# so all phone-entries have a common interface
...
=end
	def self.import_data
		...
		fp_client_google_table = ft.show_tables[10]
		...
		if last_record.nil?
			...
			new_records = fp_client_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end

		fields = fp_client_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
			...
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					...
					new_fp_client.build_detail(
						...
						:method_used=> record[fields[14][:name].downcase.to_sym],
					...
					new_fp_client.save!
					unless new_fp_client.photo_url.nil?
		...
		puts  "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " fp_client records."
	end
...
@param [Array of Objects holding the statistics] fp_client_records these objects contain the necessary statistics that will be attached. 
@return [Array of Objects implementing Reportable Module] the collection object with attached statistics 
=end
	def self.build_statistics(fp_client_records,collection)
		for unit in fp_client_records
			instance = collection.find { |instance| instance.name == unit.name }
			#attr_accessor_with_default is deprecated :S
			instance.fp_client_count_total = unit.fp_client_count_total_c.to_i
