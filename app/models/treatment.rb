...
=begin
Wraps data-collection functionality for treatments.
...
=end
class Treatment < PhoneEntry

	has_one :detail, :class_name =>  "TreatmentDetail"	# so all phone-entries have a common interface
...
=end
	def self.import_data
		...
		if last_record.nil?
			...
			new_records = treatment_google_table.select "*", "ORDER BY '*meta-submission-date*' ASC"
		else
			...
			new_records = treatment_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end

		fields = treatment_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					...
					new_treatment.save!
					#puts record.to_yaml
					#puts new_treatment.detail.epi_polio_bcg
					#puts record[fields[16][:name].downcase.to_sym].tr("T"," ")

					#unless new_treatment.photo_url.nil?
					#	new_treatment.update_attribute(:photo,open(new_treatment.photo_url))
					#end
		...
		puts  "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " treatment records."
	end
...
@param [Array of Objects holding the statistics] treatment_records these objects contain the necessary statistics that will be attached. 
@return [Array of Objects implementing Reportable Module] the collection object with attached statistics 
=end
	def self.build_statistics(treatment_records,collection)
		for unit in treatment_records
			instance = collection.find { |instance| instance.name == unit.name }
			#attr_accessor_with_default is deprecated :S
			instance.treatment_count_total = unit.treatment_count_total_c.to_i
