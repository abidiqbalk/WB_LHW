...
=begin
Wraps data-collection functionality for reporting_birth_deaths.
...
=======
		reporting_birth_death_google_table = ft.show_tables[ft.show_tables.index{|x|x.name=="Reporting - Births and Deaths"}]
>>>>>>> upstream/master
		...
		if last_record.nil?
			...
			new_records = reporting_birth_death_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end

		fields = reporting_birth_death_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					new_reporting_birth_death = self.new(
					...
					new_reporting_birth_death.save!
					#unless new_reporting_birth_death.photo_url.nil?
					#new_reporting_birth_death.update_attribute(:photo,open(new_reporting_birth_death.photo_url))
					#end
		...
		puts  "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " reporting_birth_death records."
	end
...
@param [Array of Objects holding the statistics] reporting_birth_death_records these objects contain the necessary statistics that will be attached. 
@return [Array of Objects implementing Reportable Module] the collection object with attached statistics 
=end
	def self.build_statistics(reporting_birth_death_records,collection)
		for unit in reporting_birth_death_records
			instance = collection.find { |instance| instance.name == unit.name }
			#attr_accessor_with_default is deprecated :S
			instance.reporting_birth_death_count_total = unit.reporting_birth_death_count_total_c.to_i
