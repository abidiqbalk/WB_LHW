...
class HouseholdReport < PhoneEntry

	has_one :detail, :class_name => "HouseholdReportDetail"	# so all phone-entries have a common interface

	def self.import_data
		puts "Importing house health"
		ft = GData::Client::FusionTables.new 
		ft.clientlogin(Yetting.fusion_account,Yetting.fusion_password)		
		household_report_google_table = ft.show_tables[2]
		last_record = self.order("meta_submission_date").last
		
		if last_record.nil?
			puts "nil record case got run"
			new_records = household_report_google_table.select "*", "ORDER BY '*meta-submission-date*' ASC"
		else
			last_record = self.order("meta_submission_date").last
			...
			new_records = household_report_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end
		fields = household_report_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					...
					new_household_report.build_detail(
					...
					new_household_report.save!
					unless new_household_report.photo_url.nil?
						new_household_report.update_attribute(:photo,open(new_household_report.photo_url))
					end
		...
		puts "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " household_report records."
	end
end
