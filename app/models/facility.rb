...
=begin
Wraps data-collection functionality for facilitys.
...
=end
class Facility < PhoneEntry

	has_one :detail, :class_name => "FacilityDetail"	# so all phone-entries have a common interface
...
=end
	def self.import_data
		puts  "Importing facility on #{Time.now}"
		ft = GData::Client::FusionTables.new 
		ft.clientlogin(Yetting.fusion_account,Yetting.fusion_password)		
		facility_google_table = ft.show_tables[1]
		
		last_record = self.order("meta_submission_date").last
		
		if last_record.nil?
			puts facility_google_table.inspect
			puts  "nil record case got run"
			new_records = facility_google_table.select "*", "ORDER BY '*meta-submission-date*' ASC"
		else
			...
			new_records = facility_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end

		fields = facility_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					...
					new_facility.save!
					unless new_facility.photo_url.nil?
						new_facility.update_attribute(:photo,open(new_facility.photo_url))
					end
		...
		puts  "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " facility records."
	end
...
@param [Array of Objects holding the statistics] facility_records these objects contain the necessary statistics that will be attached. 
@return [Array of Objects implementing Reportable Module] the collection object with attached statistics 
=end
	def self.build_statistics(facility_records,collection)
		for unit in facility_records
			instance = collection.find { |instance| instance.name == unit.name }
			#attr_accessor_with_default is deprecated :S
			instance.facility_count_total = unit.facility_count_total_c.to_i
...
=end
	def self.indicators(averages)
		a=Indicator.new(:name=>"Students in Grade 3 appearing for paper",:hook => "students_grade3", :entry_type => facilityDetail, :statistics_set_array => averages, :alternate_name=>"Grade 3 facility")
		b=Indicator.new(:name=>"Students in Grade 4 appearing for paper", :hook => "students_grade4", :entry_type => facilityDetail, :statistics_set_array => averages, :alternate_name=>"Grade 4 facility")
		c=Indicator.new(:name=>"Students in Grade 5 appearing for paper", :hook => "students_grade5", :entry_type => facilityDetail, :statistics_set_array => averages, :alternate_name=>"Grade 5 facility")
		d=Indicator.new(:name=>"Teachers Present", :hook => "teachers_present", :entry_type => facilityDetail, :statistics_set_array => averages, :alternate_name=>"Teacher Attendance")
		e=Indicator.new(:name=>"Tasks Identified",:hook => "tasks_identified", :entry_type => facilityDetail, :statistics_set_array => averages, :alternate_name=>"Tasks Identified for Cooperation of HT")
		return [a,b,c,d,e]
	end
