...
=begin
Wraps data-collection functionality for support_group_meetings.
...
=end
class SupportGroupMeeting < PhoneEntry

	has_one :detail, :class_name => "SupportGroupMeetingDetail"	# so all phone-entries have a common interface
...
=end
	def self.import_data
		...
		#ft.clientlogin(Yetting.fusion_account,Yetting.fusion_password)		
		support_group_meeting_google_table = ft.show_tables[ft.show_tables.index{|x|x.name=="Monitoring - Support Group Meeting"}]
		...
		if last_record.nil?			
			...
			new_records = support_group_meeting_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end

		fields = support_group_meeting_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					new_support_group_meeting = self.new(
					...
					new_support_group_meeting.build_detail(
						:lhw_code=> record[fields[11][:name].downcase.to_sym],
						:number_of_groups_formed=> record[fields[12][:name].downcase.to_sym],
						:members_in_group_a=> record[fields[13][:name].downcase.to_sym],
						:members_in_group_b => record[fields[14][:name].downcase.to_sym],
		...
		puts  "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " support_group_meeting records."
	end
...
@param [Array of Objects holding the statistics] support_group_meeting_records these objects contain the necessary statistics that will be attached. 
@return [Array of Objects implementing Reportable Module] the collection object with attached statistics 
=end
	def self.build_statistics(support_group_meeting_records,collection)
		for unit in support_group_meeting_records
			instance = collection.find { |instance| instance.name == unit.name }
			#attr_accessor_with_default is deprecated :S
			instance.support_group_meeting_count_total = unit.support_group_meeting_count_total_c.to_i
