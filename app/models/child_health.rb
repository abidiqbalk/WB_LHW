...
=begin
Wraps data-collection functionality for child_healths.
...
=end
class ChildHealth < PhoneEntry

	has_one :detail, :class_name =>  "ChildHealthDetail"	# so all phone-entries have a common interface
...
=end
	def self.import_data
		...
		#for tables in ft.show_tables
		#logger.fatal tables.name
		#end
		...
		if last_record.nil?
			...
			new_records = child_health_google_table.select "*", "WHERE '*meta-submission-date*' >= '#{search_after}' and '*meta-instance-id*' NOT EQUAL TO '#{last_record.meta_instance_id}' ORDER BY '*meta-submission-date*' ASC"
		end

		fields = child_health_google_table.describe
		success_count = 0
		fail_location = 0 
		fail_sim = 0
		duplicate_fail = 0
		...
		for record in new_records 			
			begin
				...
				unless locations.count!=3 or record["simid".downcase.to_sym].blank?
					...
					#puts record.to_yaml
					#puts new_child_health.detail.epi_polio_bcg
					#puts record[fields[16][:name].downcase.to_sym].tr("T"," ")

					unless new_child_health.photo_url.nil?
						new_child_health.update_attribute(:photo,open(new_child_health.photo_url))
					end
		...
		puts  "Imported " +  success_count.to_s + " of " + new_records.count.to_s + " child_health records."
	end
...
=end
	def self.indicators2
		...
		f=Indicator2.new(:hook => "epi_status", :indicator_activity=>self)
		g=Indicator2.new(:hook => "epi_polio_bcg", :indicator_type => "date", :indicator_activity=>self)
		h=Indicator2.new(:hook => "penta1_polio", :indicator_type => "date", :indicator_activity=>self)
		i=Indicator2.new(:hook => "penta2_polio", :indicator_type => "date", :indicator_activity=>self)
		j=Indicator2.new(:hook => "penta3_polio", :indicator_type => "date", :indicator_activity=>self)
		k=Indicator2.new(:hook => "measles1", :indicator_type => "date", :indicator_activity=>self)
		l=Indicator2.new(:hook => "measles2", :indicator_type => "date", :indicator_activity=>self)		
